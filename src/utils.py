...
def min_k_prob(model: PreTrainedModel, token_ids: torch.Tensor, attention_mask: torch.Tensor, k: int = 20):
    with torch.no_grad():
        ...
        token_logp = token_logp.float().detach().cpu().numpy()
...
def compute_perplexity(
    ...
    with torch.no_grad():
        ...
        return np.exp(loss.float().detach().cpu().numpy())
...
def compute_perplexity_df(
    ...
    for i in range(0, len(raw_traps), batch_size):
        ...
        trap_tokens_croissant = croissant_tokenizer(
            list(batch), return_tensors="pt", padding="longest"
        ).to(croissant_device)
        trap_tokens_llama = llama_tokenizer(list(batch), return_tensors="pt", padding="longest").to(
            llama_device
        )
